...
db.getUserByEmail = (email) =>{
    return new Promise((resolve, reject)=>{
        try{
            pool.query("SELECT u.uuid,TRIM(CONCAT(u.first_name,' ',IFNULL(u.last_name,''))) AS fullName,u.role_id,TRIM(r.name) AS role_name, u.user_type_id, u.last_login,u.password,u.id, u.is_active ,ut.name AS user_type_name,ut.code AS user_type_code FROM user u LEFT JOIN role r ON u.role_id = r.id LEFT JOIN user_type ut ON ut.id = u.user_type_id WHERE u.email = ? AND u.is_active =1", [email], (error, users)=>{
                if(error){
                    return reject(error);
                }
