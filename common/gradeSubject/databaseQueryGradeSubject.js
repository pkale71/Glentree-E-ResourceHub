...
db.findSubject = (name,gradeId,syllabusId) => {
    return new Promise((resolve, reject)=>{
        try
        {
            pool.query(`SELECT COUNT(subject_name) AS Exist, grade_id FROM syllabus_grade_subject WHERE  grade_id = ? AND syllabus_id = ? AND subject_name LIKE ?`, [gradeId,syllabusId,name], (error, result) => 
            {
                if(error)
                {
                    return reject(error);
...
db.checkUsedSubject = (id) => {
    return new Promise((resolve, reject)=>{
        try
        {
            pool.query(`SELECT COUNT(syllabus_grade_subject_id) AS Exist FROM syllabus_grade_subject_chaptes WHERE   syllabus_grade_subject_id LIKE ?`, [id], (error, result) => 
            {
                if(error)
                {
                    return reject(error);
